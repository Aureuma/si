...
- When cooldown is detected, `codex-monitor` can create a `codex.account_reset` task to wipe Codex state and prepare for a new login (human-in-the-loop). Disable with `CODEX_RESET_ON_COOLDOWN=0`.
