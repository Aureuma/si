...
- Codex CLI: use `configs/codex-mcp-config.toml` and place it at `~/.codex/config.toml` inside the actor/critic container (helper: `bin/apply-codex-mcp-config.sh <dyad>`).
