...
- **Telegram Bot**: Go control-plane service listening on `:8081/notify` to push human-queue items to Telegram. Uses `TELEGRAM_BOT_TOKEN` (from secret) and `TELEGRAM_CHAT_ID` (env) to deliver messages.
