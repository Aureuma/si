...
- **Compose limits**: core services have CPU/memory limits set in `docker-compose.yml` to prevent runaway usage:
...
- **Best practices**
  - Keep per-app DBs lean; stop unused containers (`docker compose stop <service>`).
  - Prefer per-app limits when spawning additional dyads; inherit limits from base compose.
