...
### Workflow
- Discover: list catalogs and servers from the MCP Gateway (`silexa mcp scout`).
...
- Adopt: infra or relevant dyad updates `/catalog/catalog.yaml` (volume-backed) or adds via `docker-mcp catalog add`, then restarts the gateway (`silexa stack up`).

### Tooling
- `silexa mcp scout`: quick explorer to show catalogs and top entries from the default docker-mcp catalog. Intended for exploratory runs by the exploration unit.

### Guards
- Do not enable servers that require broad secrets without security approval.
- Note runtime impact (memory, CPU) if a server is heavy; apply Docker limits when adding.
