...
### Workflow
- Discover: list catalogs and servers from the MCP Gateway (`docker compose run --rm mcp-gateway catalog ls|show`).
...
- Adopt: infra or relevant dyad mounts catalog entries into `/data/mcp-gateway/catalog.yaml` (or adds via `docker-mcp catalog add`) and restarts gateway.
...
- Note runtime impact (memory, CPU) if a server is heavy; apply compose limits when adding.
