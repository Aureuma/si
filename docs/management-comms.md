...
- **Periodic reports**: `silexa report status` and `silexa report review` for scheduled summaries; send to Telegram.

### Broadcast helper
- Use `silexa feedback broadcast "<message>" [severity]` to fan out: it posts to manager feedback and optionally Telegram (if `TELEGRAM_NOTIFY_URL` is set).
