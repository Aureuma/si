...
```bash
si oci identity domains list --tenancy <tenancy_ocid> --json
...
si oci network gateway create --compartment <ocid> --vcn-id <vcn_ocid> --display-name igw --enabled --json
si oci network security create --compartment <ocid> --vcn-id <vcn_ocid> --ssh-port 22 --json
...
```bash
si oci compute image ubuntu --tenancy <ocid> --shape VM.Standard.E4.Flex --json
...
```bash
si oci oracular cloudinit --ssh-port 22 --json
