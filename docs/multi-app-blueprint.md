...
apps/<app>/
  ...
  docs/
    plan.md            # generated by silexa app init
...
1) Bootstrap the app:
   ...
   - Existing apps: `silexa app adopt <app> --web-path <path> [--backend-path <path>]`.
2) Register dyads:
   - `silexa dyad register <app> <role> <department>`
3) Route tasks:
   - Add app keywords to `configs/router_rules.json` as needed.
4) Provision DB:
   - Create the database in your chosen Postgres and store creds in `secrets/app-<app>.env`.
...
### Deployment
- Each app deploys via Docker Compose: `silexa app deploy <app>`.
- See `docs/app-deploy.md` for build/secrets/port overrides.
- Check live stack health: `silexa app status <app>`.
