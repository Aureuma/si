...
- completed: the GitHub provider slice now also owns `git credential get`, with Rust-owned stdin request parsing, host filtering, and auth token resolution while the broader git setup/remote-auth/clone-auth helper flows remain on the Go side
