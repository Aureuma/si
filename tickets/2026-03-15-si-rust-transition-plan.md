...
- completed: the release workflow and checked-in Homebrew core artifact now follow the cutover too, with `.github/workflows/cli-release-assets.yml` provisioning Rust for the Rust-owned release helpers and building Rust-primary fallback archives plus `si-go`, and `packaging/homebrew-core/si.rb` regenerated through the Rust renderer for the current release tag
