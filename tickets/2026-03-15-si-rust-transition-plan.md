...
- completed: the Google provider slice now also owns the initial Google Play runtime and metadata lane (`context list|current`, `auth status`, default signed `doctor`, `raw`, custom-app `app create`, `listing get|list|update`, and `details get|update`), with Rust-owned service-account JWT exchange, Android Publisher/custom-app transport, edit lifecycle handling, and Go wrapper delegation while the separate `doctor --public` probe plus the broader Play asset/release/apply flows remain on the Go path for now
