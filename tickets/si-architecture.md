...
- `[implementation-note:nucleus-primary-id-boundary-coverage-2026-04-06]` The SI primary-id boundary is now pinned at runtime-backed state too. `si-nucleus` regression coverage asserts that worker, session, and run ids remain namespaced (`si-worker-`, `si-session-`, `si-run-`) while the persisted App Server thread and turn ids stay distinct runtime-side identifiers instead of becoming SI primary ids.
