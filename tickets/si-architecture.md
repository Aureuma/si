...
- `[implementation-note:nucleus-profile-routing-mismatch-2026-04-06]` Session-affine routing now rejects cross-profile reuse explicitly in the queued dispatch path too. `si-nucleus` now blocks a task with `session_broken` when the task’s explicit profile does not match the bound profile of its referenced session, and regression coverage asserts that Nucleus preserves the original session profile instead of silently rebinding that session into a different account lane.
