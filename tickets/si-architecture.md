...
- `[implementation-note:nucleus-phase5-live-task-prune-coverage-2026-04-06]` The explicit `si nucleus task prune` operator path now also has live service coverage. Against a running Nucleus service, the CLI prune command removes aged terminal tasks while leaving an equally old queued backlog task and recent terminal task intact, which pins the documented old-terminal-tasks-only retention boundary through the public control-plane surface.
...
- `[implementation-note:nucleus-phase6-live-fort-coverage-2026-04-06]` Fort-backed execution now also has a live service acceptance smoke through the documented public Fort CLI surface. A task that references the documented `si fort` contract is executed against a running Nucleus instance after `si fort session write --path <codex_home>/fort/session.json --state-json ...` persists the worker-visible Fort session, and the test verifies both successful task completion and durable `fort.ready` event projection in the canonical event ledger.
- `[implementation-note:nucleus-phase6-live-fort-failure-coverage-2026-04-06]` The live Fort matrix now also covers failure paths through the same documented surface. A Fort-referencing task blocks with `auth_required` after `si fort session clear --path <codex_home>/fort/session.json` removes a previously valid worker-visible Fort session, a second task blocks with `fort_unavailable` when the persisted Fort session state is malformed, and the direct `si nucleus run submit-turn` surface now pins the same malformed-session `fort_unavailable` branch while leaving the task unclaimed. All of those paths are exercised against a running Nucleus service and assert the matching durable canonical Fort events.
- `[implementation-note:nucleus-phase6-live-fort-cli-lifecycle-coverage-2026-04-06]` The live Fort matrix now also pins the public Fort session lifecycle end to end against one running Nucleus instance. The same worker-visible Fort path is exercised through `si fort session write`, `si fort session show`, and `si fort session clear`, and Nucleus is then verified to complete a Fort-backed task while the session is present and block a second Fort-backed task with `auth_required` immediately after the public clear, while projecting the matching `fort.ready` and `fort.auth_required` canonical events.
...
- `[implementation-note:nucleus-openapi-live-info-coverage-2026-04-06]` The live `/openapi.json` smoke now also pins the top-level OpenAPI `info` object directly. Against a running service, the document is asserted to keep the canonical `SI Nucleus REST API` title, the current SI release version, and the bounded external integration description, so the published discovery metadata stays aligned with the intended architecture and not just with the path-level operation contract.
- `[implementation-note:nucleus-openapi-unit-info-coverage-2026-04-06]` The unit-level OpenAPI regression now also pins the top-level `info` object directly, matching the live `/openapi.json` smoke. Tests now assert the canonical title, current SI release version, and bounded external integration description at the document root, so both unit and live coverage enforce the same discovery metadata contract.
- `[implementation-note:nucleus-openapi-live-security-type-coverage-2026-04-06]` The live `/openapi.json` smoke now also pins the remaining low-level bearer-auth metadata directly. Against a running service, the document is asserted to keep `components.securitySchemes.bearerAuth.type = "http"` alongside the existing `scheme` and `bearerFormat` checks, so the published auth metadata stays aligned with the intended HTTP bearer contract and not just with the write-operation security references.
- `[implementation-note:nucleus-openapi-live-server-url-coverage-2026-04-06]` The live `/openapi.json` smoke now also pins the published top-level server URL directly. Against a running service, the document is asserted to keep `servers[0].url` on the canonical relative server contract, so the discovery metadata stays aligned with the running service without exposing wildcard bind addresses as client endpoints.
- `[implementation-note:nucleus-openapi-relative-server-url-2026-04-06]` The bounded OpenAPI document now advertises a relative server URL (`/`) instead of echoing the raw bind address. This avoids publishing wildcard listen addresses like `0.0.0.0` as client endpoints while still keeping the document usable from the same origin, and both unit and live OpenAPI regression coverage now pin that relative server contract directly.
...
- `[implementation-note:nucleus-rest-openapi-post-server-fix-verification-2026-04-06]` After fixing the relative OpenAPI server URL and tightening the remaining live metadata checks, the bounded REST/OpenAPI verification slices remain green. `cargo test -p si-rs-cli nucleus_live_openapi_document_advertises_bounded_contract -- --nocapture`, `cargo test -p si-rs-cli nucleus_live_rest_ -- --nocapture`, and `cargo test -p si-nucleus rest_openapi_document_describes_bounded_external_endpoints -- --nocapture` all pass together, confirming the running service, live OpenAPI document, and unit-level bounded contract stayed aligned through the last conformance fixes.
...
- `[implementation-note:nucleus-unroutable-task-blocking-2026-04-16]` Unroutable queued tasks now surface explicit operator state instead of remaining silently queued. Missing referenced sessions project to `session_broken`, tasks without an inferable profile project to `profile_unavailable`, and recoverable profile-unavailable tasks can requeue once a single profile lane becomes unambiguous.
- `[implementation-note:nucleus-state-owner-lock-2026-04-16]` Nucleus now acquires an exclusive state-directory lock before opening durable state, writes gateway metadata only after the listener bind succeeds, and rejects accidental unsupported `si-nucleus` arguments before runtime loops can start. This closes the cross-process writer class that previously allowed duplicate state-dir owners to corrupt the canonical JSONL ledger.
- `[implementation-note:nucleus-cleanup-replay-prune-2026-04-16]` Hook processing now returns before event-log replay when no hook rules exist, so empty hook configuration is not a global dependency on event-ledger readability. `task.prune` also treats cancelled tasks as terminal cleanup candidates, keeping cancellation semantics aligned with the task-state model.
- `[verification-note:nucleus-plan-audit-2026-04-16]` The ticket directory was re-audited against the current SI implementation. No planned or active phase remains open; the live service was verified on the current Nucleus build with ready workers, no non-terminal backlog, and clean event JSONL parsing.
...
- completed, failed, and cancelled tasks remain inspectable until explicitly pruned
- use a conservative default retention window such as 30 days for completed, failed, and cancelled tasks unless explicitly configured otherwise
