...
func main() {
	...
	fmt.Println("==> non-root smoke: install into user default path")
	if err := run(installer, "--source-dir", sourceDir, "--force", "--no-buildx", "--quiet"); err != nil {
		os.Exit(1)
	}
	...
	fmt.Println("==> non-root smoke: uninstall")
	if err := run(installer, "--uninstall", "--quiet"); err != nil {
		os.Exit(1)
	}
