...
func TestRunBuildSelfCommandDefaultsToGoForNonReleaseAssets(t *testing.T) {
	t.Setenv(siExperimentalRustCLIEnv, "")
	t.Setenv(siRustCLIBinEnv, "")

	delegated, err := runBuildSelfCommand([]string{"build"})
	if err != nil {
		t.Fatalf("runBuildSelfCommand: %v", err)
	}
	if delegated {
		t.Fatalf("expected Go build self path for non-release-assets subcommands")
	}
}

func TestRunBuildSelfCommandDelegatesReleaseAssetsToRustCLIWhenConfigured(t *testing.T) {
	dir := t.TempDir()
	argsPath := filepath.Join(dir, "args.txt")
	scriptPath := filepath.Join(dir, "si-rs")
	script := "#!/bin/sh\nprintf '%s\\n' 'rust-release-assets'\nprintf '%s\\n' \"$@\" >" + shellSingleQuote(argsPath) + "\n"
	...
	out := captureOutputForTest(t, func() {
		delegated, err := runBuildSelfCommand([]string{"release-assets", "--repo", "/tmp/si", "--out-dir", "/tmp/out"})
		if err != nil {
			t.Fatalf("runBuildSelfCommand: %v", err)
		}
		if !delegated {
			t.Fatalf("expected build self release-assets to delegate to Rust")
		}
	})

	if strings.TrimSpace(out) != "rust-release-assets" {
		t.Fatalf("expected delegated Rust output, got %q", out)
	}
	argsData, err := os.ReadFile(argsPath)
	if err != nil {
	...
	if strings.TrimSpace(string(argsData)) != "build\nself\nrelease-assets\n--repo\n/tmp/si\n--out-dir\n/tmp/out" {
		t.Fatalf("expected Rust CLI args to be build self release-assets + flags, got %q", string(argsData))
	}
}
...
func TestRunAWSS3CommandDelegatesBucketListToRustCLIWhenConfigured(t *testing.T) {
	dir := t.TempDir()
	argsPath := filepath.Join(dir, "args.txt")
	scriptPath := filepath.Join(dir, "si-rs")
	script := "#!/bin/sh\nprintf '%s\\n' 'rust-aws-s3'\nprintf '%s\\n' \"$@\" >" + shellSingleQuote(argsPath) + "\n"
	...
	out := captureOutputForTest(t, func() {
		...
		if !delegated {
			t.Fatalf("expected aws s3 bucket list to delegate to Rust")
		}
	})

	if strings.TrimSpace(out) != "rust-aws-s3" {
		t.Fatalf("expected delegated Rust aws s3 output, got %q", out)
	}
	argsData, err := os.ReadFile(argsPath)
	if err != nil {
		t.Fatalf("read args file: %v", err)
	...
	if strings.TrimSpace(string(argsData)) != "aws\ns3\nbucket\nlist\n--json" {
		t.Fatalf("expected Rust CLI args to be aws s3 bucket + args, got %q", string(argsData))
	}
}

func TestRunAWSEC2CommandDelegatesInstanceListToRustCLIWhenConfigured(t *testing.T) {
	dir := t.TempDir()
	argsPath := filepath.Join(dir, "args.txt")
	scriptPath := filepath.Join(dir, "si-rs")
	script := "#!/bin/sh\nprintf '%s\\n' 'rust-aws-ec2'\nprintf '%s\\n' \"$@\" >" + shellSingleQuote(argsPath) + "\n"
	...
	out := captureOutputForTest(t, func() {
		...
		if !delegated {
			t.Fatalf("expected aws ec2 instance list to delegate to Rust")
		}
	})

	if strings.TrimSpace(out) != "rust-aws-ec2" {
		t.Fatalf("expected delegated Rust aws ec2 output, got %q", out)
	}
	argsData, err := os.ReadFile(argsPath)
	if err != nil {
		t.Fatalf("read args file: %v", err)
	...
	if strings.TrimSpace(string(argsData)) != "aws\nec2\ninstance\nlist\n--json" {
		t.Fatalf("expected Rust CLI args to be aws ec2 instance + args, got %q", string(argsData))
	}
}

func TestRunAWSLambdaCommandDelegatesFunctionListToRustCLIWhenConfigured(t *testing.T) {
	dir := t.TempDir()
	argsPath := filepath.Join(dir, "args.txt")
	scriptPath := filepath.Join(dir, "si-rs")
	script := "#!/bin/sh\nprintf '%s\\n' 'rust-aws-lambda'\nprintf '%s\\n' \"$@\" >" + shellSingleQuote(argsPath) + "\n"
	...
	out := captureOutputForTest(t, func() {
		...
		if !delegated {
			t.Fatalf("expected aws lambda function list to delegate to Rust")
		}
	})

	if strings.TrimSpace(out) != "rust-aws-lambda" {
		t.Fatalf("expected delegated Rust aws lambda output, got %q", out)
	}
	argsData, err := os.ReadFile(argsPath)
	if err != nil {
		t.Fatalf("read args file: %v", err)
	...
	if strings.TrimSpace(string(argsData)) != "aws\nlambda\nfunction\nlist\n--json" {
		t.Fatalf("expected Rust CLI args to be aws lambda function + args, got %q", string(argsData))
	}
}

func TestRunAWSECRCommandDelegatesRepositoryListToRustCLIWhenConfigured(t *testing.T) {
	dir := t.TempDir()
	argsPath := filepath.Join(dir, "args.txt")
	scriptPath := filepath.Join(dir, "si-rs")
	script := "#!/bin/sh\nprintf '%s\\n' 'rust-aws-ecr'\nprintf '%s\\n' \"$@\" >" + shellSingleQuote(argsPath) + "\n"
	...
	out := captureOutputForTest(t, func() {
		...
		if !delegated {
			t.Fatalf("expected aws ecr repository list to delegate to Rust")
		}
	})

	if strings.TrimSpace(out) != "rust-aws-ecr" {
		t.Fatalf("expected delegated Rust aws ecr output, got %q", out)
	}
	argsData, err := os.ReadFile(argsPath)
	if err != nil {
		t.Fatalf("read args file: %v", err)
	...
	if strings.TrimSpace(string(argsData)) != "aws\necr\nrepository\nlist\n--json" {
		t.Fatalf("expected Rust CLI args to be aws ecr repository + args, got %q", string(argsData))
	}
}
...
func TestRunAWSDynamoDBCommandDelegatesTableListToRustCLIWhenConfigured(t *testing.T) {
	dir := t.TempDir()
	argsPath := filepath.Join(dir, "args.txt")
	scriptPath := filepath.Join(dir, "si-rs")
	script := "#!/bin/sh\nprintf '%s\\n' 'rust-aws-dynamodb'\nprintf '%s\\n' \"$@\" >" + shellSingleQuote(argsPath) + "\n"
	...
	out := captureOutputForTest(t, func() {
		...
		if !delegated {
			t.Fatalf("expected aws dynamodb table list to delegate to Rust")
		}
	})

	if strings.TrimSpace(out) != "rust-aws-dynamodb" {
		t.Fatalf("expected delegated Rust aws dynamodb output, got %q", out)
	}
	argsData, err := os.ReadFile(argsPath)
	if err != nil {
		t.Fatalf("read args file: %v", err)
	...
	if strings.TrimSpace(string(argsData)) != "aws\ndynamodb\ntable\nlist\n--json" {
		t.Fatalf("expected Rust CLI args to be aws dynamodb + args, got %q", string(argsData))
	}
}

func TestRunAWSSSMCommandDelegatesParameterListToRustCLIWhenConfigured(t *testing.T) {
	dir := t.TempDir()
	argsPath := filepath.Join(dir, "args.txt")
	scriptPath := filepath.Join(dir, "si-rs")
	script := "#!/bin/sh\nprintf '%s\\n' 'rust-aws-ssm'\nprintf '%s\\n' \"$@\" >" + shellSingleQuote(argsPath) + "\n"
	...
	out := captureOutputForTest(t, func() {
		...
		if !delegated {
			t.Fatalf("expected aws ssm parameter list to delegate to Rust")
		}
	})

	if strings.TrimSpace(out) != "rust-aws-ssm" {
		t.Fatalf("expected delegated Rust aws ssm output, got %q", out)
	}
	argsData, err := os.ReadFile(argsPath)
	if err != nil {
		t.Fatalf("read args file: %v", err)
	...
	if strings.TrimSpace(string(argsData)) != "aws\nssm\nparameter\nlist\n--json" {
		t.Fatalf("expected Rust CLI args to be aws ssm + args, got %q", string(argsData))
	}
}

func TestRunAWSLogsCommandDelegatesGroupListToRustCLIWhenConfigured(t *testing.T) {
	dir := t.TempDir()
	argsPath := filepath.Join(dir, "args.txt")
	scriptPath := filepath.Join(dir, "si-rs")
	script := "#!/bin/sh\nprintf '%s\\n' 'rust-aws-logs'\nprintf '%s\\n' \"$@\" >" + shellSingleQuote(argsPath) + "\n"
	...
	out := captureOutputForTest(t, func() {
		...
		if !delegated {
			t.Fatalf("expected aws logs group list to delegate to Rust")
		}
	})

	if strings.TrimSpace(out) != "rust-aws-logs" {
		t.Fatalf("expected delegated Rust aws logs output, got %q", out)
	}
	argsData, err := os.ReadFile(argsPath)
	if err != nil {
		t.Fatalf("read args file: %v", err)
	...
	if strings.TrimSpace(string(argsData)) != "aws\nlogs\ngroup\nlist\n--json" {
		t.Fatalf("expected Rust CLI args to be aws logs + args, got %q", string(argsData))
	}
}

func TestRunAWSCloudWatchCommandDelegatesMetricListToRustCLIWhenConfigured(t *testing.T) {
	dir := t.TempDir()
	argsPath := filepath.Join(dir, "args.txt")
	scriptPath := filepath.Join(dir, "si-rs")
	script := "#!/bin/sh\nprintf '%s\\n' 'rust-aws-cloudwatch'\nprintf '%s\\n' \"$@\" >" + shellSingleQuote(argsPath) + "\n"
	...
	out := captureOutputForTest(t, func() {
		...
		if !delegated {
			t.Fatalf("expected aws cloudwatch metric to delegate to Rust")
		}
	})

	if strings.TrimSpace(out) != "rust-aws-cloudwatch" {
		t.Fatalf("expected delegated Rust aws cloudwatch output, got %q", out)
	}
	argsData, err := os.ReadFile(argsPath)
	if err != nil {
		t.Fatalf("read args file: %v", err)
	...
	if strings.TrimSpace(string(argsData)) != "aws\ncloudwatch\nmetric\n--json" {
		t.Fatalf("expected Rust CLI args to be aws cloudwatch + args, got %q", string(argsData))
	}
}
...
func TestRunAWSBedrockCommandDelegatesRuntimeInvokeToRustCLIWhenConfigured(t *testing.T) {
	dir := t.TempDir()
	argsPath := filepath.Join(dir, "args.txt")
	scriptPath := filepath.Join(dir, "si-rs")
	script := "#!/bin/sh\nprintf '%s\\n' 'rust-aws-bedrock-runtime'\nprintf '%s\\n' \"$@\" >" + shellSingleQuote(argsPath) + "\n"
	if err := os.WriteFile(scriptPath, []byte(script), 0o700); err != nil {
		t.Fatalf("write script: %v", err)
	}
	...
	out := captureOutputForTest(t, func() {
		...
		if !delegated {
			t.Fatalf("expected aws bedrock runtime invoke to delegate to Rust")
		}
	})

	if strings.TrimSpace(out) != "rust-aws-bedrock-runtime" {
		t.Fatalf("expected delegated Rust aws bedrock runtime output, got %q", out)
	}
	argsData, err := os.ReadFile(argsPath)
	if err != nil {
		t.Fatalf("read args file: %v", err)
	...
	if strings.TrimSpace(string(argsData)) != "aws\nbedrock\nruntime\ninvoke\n--model-id\nm-1\n--prompt\nhello\n--json" {
		t.Fatalf("expected Rust CLI args to be aws bedrock runtime + args, got %q", string(argsData))
	}
}
...
func TestRunGoogleCommandDelegatesToRustCLIForYouTubeReadPath(t *testing.T) {
	...
	out := captureOutputForTest(t, func() {
		...
		if !delegated {
			t.Fatalf("expected google root to delegate youtube search list to Rust")
		}
	})

	if strings.TrimSpace(out) != "rust-google-youtube-root" {
		t.Fatalf("expected delegated Rust google youtube root output, got %q", out)
	}
	argsData, err := os.ReadFile(argsPath)
	if err != nil {
		t.Fatalf("read args file: %v", err)
	...
	if strings.TrimSpace(string(argsData)) != "google\nyoutube\nsearch\nlist\n--query\nmusic\n--json" {
		t.Fatalf("expected Rust CLI args to be google youtube search list + args, got %q", string(argsData))
	}
}
...
func TestRunGoogleYouTubeDoctorCommandDefaultsToGoForPublicProbe(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("runGoogleYouTubeDoctorCommand: %v", err)
	}
	if delegated {
		t.Fatalf("expected Go google youtube doctor path for --public")
	}
}

func TestRunGoogleYouTubeDoctorCommandDelegatesToRustCLIWhenConfigured(t *testing.T) {
	dir := t.TempDir()
	argsPath := filepath.Join(dir, "args.txt")
	scriptPath := filepath.Join(dir, "si-rs")
	script := "#!/bin/sh\nprintf '%s\\n' 'rust-google-youtube-doctor'\nprintf '%s\\n' \"$@\" >" + shellSingleQuote(argsPath) + "\n"
	if err := os.WriteFile(scriptPath, []byte(script), 0o700); err != nil {
		t.Fatalf("write script: %v", err)
	}
	...
	out := captureOutputForTest(t, func() {
		...
		if !delegated {
			t.Fatalf("expected google youtube doctor to delegate to Rust")
		}
	})

	if strings.TrimSpace(out) != "rust-google-youtube-doctor" {
		t.Fatalf("expected delegated Rust google youtube doctor output, got %q", out)
	}
	argsData, err := os.ReadFile(argsPath)
	if err != nil {
		t.Fatalf("read args file: %v", err)
	...
	if strings.TrimSpace(string(argsData)) != "google\nyoutube\ndoctor\n--json" {
		t.Fatalf("expected Rust CLI args to be google youtube doctor + args, got %q", string(argsData))
	}
}

func TestRunGoogleYouTubeSupportDelegatesToRustCLIWhenConfigured(t *testing.T) {
	dir := t.TempDir()
	argsPath := filepath.Join(dir, "args.txt")
	scriptPath := filepath.Join(dir, "si-rs")
	script := "#!/bin/sh\nprintf '%s\\n' 'rust-google-youtube-support'\nprintf '%s\\n' \"$@\" >" + shellSingleQuote(argsPath) + "\n"
	if err := os.WriteFile(scriptPath, []byte(script), 0o700); err != nil {
		t.Fatalf("write script: %v", err)
	}
	...
	out := captureOutputForTest(t, func() {
		...
		if err != nil {
			t.Fatalf("runGoogleYouTubeSupportCommand: %v", err)
		}
		if !delegated {
			t.Fatalf("expected google youtube support to delegate to Rust")
		}
	})

	if strings.TrimSpace(out) != "rust-google-youtube-support" {
		t.Fatalf("expected delegated Rust google youtube support output, got %q", out)
	}
	argsData, err := os.ReadFile(argsPath)
	if err != nil {
		t.Fatalf("read args file: %v", err)
	...
	if strings.TrimSpace(string(argsData)) != "google\nyoutube\nsupport\nlanguages\n--json" {
		t.Fatalf("expected Rust CLI args to be google youtube support languages + args, got %q", string(argsData))
	}
}
...
func TestRunGoogleYouTubeMediaDelegatesToRustCLIWhenConfigured(t *testing.T) {
	dir := t.TempDir()
	argsPath := filepath.Join(dir, "args.txt")
	scriptPath := filepath.Join(dir, "si-rs")
	script := "#!/bin/sh\nprintf '%s\\n' 'rust-google-youtube-media'\nprintf '%s\\n' \"$@\" >" + shellSingleQuote(argsPath) + "\n"
	if err := os.WriteFile(scriptPath, []byte(script), 0o700); err != nil {
		t.Fatalf("write script: %v", err)
	}
	...
	cases := []struct {
		...
		{
			name: "video upload",
			args: []string{"video", "upload", "--file", "clip.mp4", "--json"},
			want: "google\nyoutube\nvideo\nupload\n--file\nclip.mp4\n--json",
		},
		{
			name: "caption download",
			args: []string{"caption", "download", "--id", "cap1", "--output", "captions.vtt", "--json"},
			want: "google\nyoutube\ncaption\ndownload\n--id\ncap1\n--output\ncaptions.vtt\n--json",
		},
		{
			name: "thumbnail set",
			args: []string{"thumbnail", "set", "--video-id", "v1", "--file", "thumb.jpg", "--json"},
			want: "google\nyoutube\nthumbnail\nset\n--video-id\nv1\n--file\nthumb.jpg\n--json",
		},
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			out := captureOutputForTest(t, func() {
				...
				if !delegated {
					t.Fatalf("expected google youtube media command to delegate to Rust for %s", tc.name)
				}
			})

			if strings.TrimSpace(out) != "rust-google-youtube-media" {
				t.Fatalf("expected delegated Rust google youtube media output, got %q", out)
			}
			argsData, err := os.ReadFile(argsPath)
			if err != nil {
				t.Fatalf("read args file: %v", err)
