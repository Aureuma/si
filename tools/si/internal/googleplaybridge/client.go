...
func NewClient(cfg ClientConfig) (*Client, error) {
	if cfg.TokenProvider == nil {
		return nil, fmt.Errorf("google play token provider is required")
	}
	spec := providers.Resolve(providers.GooglePlay)
	if strings.TrimSpace(cfg.BaseURL) == "" {
		cfg.BaseURL = spec.BaseURL
	}
	if strings.TrimSpace(cfg.UploadBaseURL) == "" {
		cfg.UploadBaseURL = spec.UploadBaseURL
	}
	if strings.TrimSpace(cfg.CustomAppBaseURL) == "" {
		cfg.CustomAppBaseURL = "https://playcustomapp.googleapis.com"
	}
...
func (c *Client) Do(ctx context.Context, req Request) (Response, error) {
	if c == nil {
		return Response{}, fmt.Errorf("google play client is not initialized")
	}
	...
	return integrationruntime.DoHTTP(ctx, integrationruntime.HTTPExecutorOptions[Response]{
		...
		Client:       c.httpClient,
		DisableCache: c.cfg.DisableCache || req.UseUpload || req.MediaPath != "",
...
func (c *Client) resolveRequestURL(req Request) (string, error) {
	...
	if req.UseCustomAppBase {
		base = c.cfg.CustomAppBaseURL
	}
...
func (c *Client) buildRequest(ctx context.Context, method, endpoint string, req Request) (*http.Request, error) {
	...
	if strings.TrimSpace(req.MediaPath) != "" {
		path := filepath.Clean(strings.TrimSpace(req.MediaPath))
	...
	if strings.TrimSpace(tok.Value) == "" {
		return nil, fmt.Errorf("google play token provider returned empty token")
	}
	...
	if bodyReader != nil {
		...
		if contentType == "" {
			if strings.TrimSpace(req.MediaPath) != "" {
				contentType = detectContentTypeForPath(req.MediaPath)
			} else {
	...
	case map[string]any:
		...
		if out.List == nil {
			if images, ok := typed["images"].([]any); ok {
				out.List = anySliceToMaps(images)
			}
		}
		if out.List == nil {
			if tracks, ok := typed["tracks"].([]any); ok {
				out.List = anySliceToMaps(tracks)
			}
		}
		if out.List == nil {
			if listings, ok := typed["listings"].([]any); ok {
				out.List = anySliceToMaps(listings)
			}
		}
		return out
	default:
...
func (c *Client) ListAll(ctx context.Context, req Request, maxPages int, tokenField string) ([]map[string]any, error) {
	if c == nil {
		return nil, fmt.Errorf("google play client is not initialized")
	}
	if maxPages <= 0 {
		maxPages = 10
	}
	...
	for page := 1; page <= maxPages; page++ {
		resp, err := c.Do(ctx, Request{
			...
			UseCustomAppBase: req.UseCustomAppBase,
...
func detectContentTypeForPath(path string) string {
	path = strings.TrimSpace(path)
	if path == "" {
