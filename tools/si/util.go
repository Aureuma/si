...
vault:
  ...
  Target selection (most commands):
    ...
    Guardrail: implicit cross-repo defaults emit warnings; set SI_VAULT_STRICT_TARGET_SCOPE=1 to fail-fast, or SI_VAULT_ALLOW_CROSS_REPO=1 to silence.
	  ...
	  si vault backend use --mode <git|helia|dual>

  Sync backend policy:
    SI_VAULT_SYNC_BACKEND (or vault.sync_backend) controls vault cloud behavior:
      git: local/git-based only (default)
      dual: local/git-based + best-effort Helia backup
      helia: Helia backup required for mutating vault commands
...
plugins:
  ...
  si plugins policy show|set ... [--json]
	...
	-------------------------------
	  ...
	  SI_BROWSER_NETWORK, SI_BROWSER_CONTAINER, SI_BROWSER_MCP_PORT
	  SI_BROWSER_MCP_URL_INTERNAL, SI_BROWSER_MCP_DISABLED
