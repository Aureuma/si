...
func TestRefreshCodexProfileFortSessionLockedDelegatesRefreshTransitionToRustCLIWhenConfigured(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("SI_FORT_ALLOW_INSECURE_HOST", "1")
	profile := codexProfile{ID: "alpha"}
	...
	if boot.ProfileID != profile.ID || boot.SessionID != "sess-1" {
		t.Fatalf("unexpected refreshed bootstrap: %+v", boot)
	}
	argsData, err := os.ReadFile(argsPath)
	if err != nil {
		t.Fatalf("read args file: %v", err)
...
func TestRefreshCodexProfileFortSessionLockedUsesRustBootstrapViewForHostResolution(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("SI_FORT_ALLOW_INSECURE_HOST", "1")
	profile := codexProfile{ID: "alpha"}
	...
	argsPath := filepath.Join(dir, "args.txt")
	scriptPath := filepath.Join(dir, "si-rs")
	script := "#!/bin/sh\nprintf '%s\\n' \"$@\" >>" + shellSingleQuote(argsPath) + "\nif [ \"$1\" = \"fort\" ] && [ \"$2\" = \"session-state\" ] && [ \"$3\" = \"show\" ]; then\n  printf '%s\\n' '{\"profile_id\":\"alpha\",\"agent_id\":\"si-codex-alpha\",\"session_id\":\"sess-1\",\"host\":\"\",\"container_host\":\"\",\"access_token_path\":\"" + strings.ReplaceAll(paths.AccessTokenHostPath, "\\", "\\\\") + "\",\"refresh_token_path\":\"" + strings.ReplaceAll(paths.RefreshTokenHostPath, "\\", "\\\\") + "\",\"refresh_expires_at\":\"2030-02-01T00:00:00Z\"}'\n  exit 0\nfi\nif [ \"$1\" = \"fort\" ] && [ \"$2\" = \"session-state\" ] && [ \"$3\" = \"bootstrap-view\" ]; then\n  printf '%s\\n' '{\"profile_id\":\"alpha\",\"agent_id\":\"si-codex-alpha\",\"session_id\":\"sess-1\",\"host_url\":\"" + srv.URL + "\",\"container_host_url\":\"" + srv.URL + "\",\"access_token_path\":\"" + strings.ReplaceAll(paths.AccessTokenHostPath, "\\", "\\\\") + "\",\"refresh_token_path\":\"" + strings.ReplaceAll(paths.RefreshTokenHostPath, "\\", "\\\\") + "\",\"access_token_container_path\":\"" + strings.ReplaceAll(paths.AccessTokenContainerPath, "\\", "\\\\") + "\",\"refresh_token_container_path\":\"" + strings.ReplaceAll(paths.RefreshTokenContainerPath, "\\", "\\\\") + "\"}'\n  exit 0\nfi\nif [ \"$1\" = \"fort\" ] && [ \"$2\" = \"session-state\" ] && [ \"$3\" = \"refresh-outcome\" ]; then\n  printf '%s\\n' '{\"state\":{\"profile_id\":\"alpha\",\"agent_id\":\"si-codex-alpha\",\"session_id\":\"sess-1\",\"host\":\"" + srv.URL + "\",\"container_host\":\"" + srv.URL + "\",\"access_token_path\":\"" + strings.ReplaceAll(paths.AccessTokenHostPath, "\\", "\\\\") + "\",\"refresh_token_path\":\"" + strings.ReplaceAll(paths.RefreshTokenHostPath, "\\", "\\\\") + "\",\"access_expires_at\":\"2030-01-01T00:00:00Z\",\"refresh_expires_at\":\"2030-02-01T00:00:00Z\"},\"classification\":{\"state\":\"resumable\"}}'\n  exit 0\nfi\nif [ \"$1\" = \"fort\" ] && [ \"$2\" = \"session-state\" ] && [ \"$3\" = \"write\" ]; then\n  exit 0\nfi\nexit 1\n"
	if err := os.WriteFile(scriptPath, []byte(script), 0o700); err != nil {
		t.Fatalf("write script: %v", err)
	}
	t.Setenv(siRustCLIBinEnv, scriptPath)
...
func TestRefreshCodexProfileFortSessionLockedPersistsRustRevocationOnUnauthorized(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("SI_FORT_ALLOW_INSECURE_HOST", "1")
	profile := codexProfile{ID: "alpha"}
...
func TestEnsureUsableCodexProfileFortSessionHonorsRustRevokedClassification(t *testing.T) {
	...
	if err := writeSecretFile(paths.AccessTokenHostPath, "expired-access-token"); err != nil {
		t.Fatalf("write access token: %v", err)
	}
	if err := writeSecretFile(paths.RefreshTokenHostPath, "refresh-token-old"); err != nil {
		t.Fatalf("write refresh token: %v", err)
	}
	state := fortProfileSessionState{
		ProfileID:        "alpha",
		...
		AccessTokenPath:  paths.AccessTokenHostPath,
		RefreshTokenPath: paths.RefreshTokenHostPath,
		AccessExpiresAt:  "1970-01-01T00:00:00Z",
	...
	argsPath := filepath.Join(dir, "args.txt")
	scriptPath := filepath.Join(dir, "si-rs")
	script := "#!/bin/sh\nprintf '%s\\n' \"$@\" >>" + shellSingleQuote(argsPath) + "\nif [ \"$1\" = \"fort\" ] && [ \"$2\" = \"session-state\" ] && [ \"$3\" = \"show\" ]; then\n  printf '%s\\n' '{\"profile_id\":\"alpha\",\"agent_id\":\"si-codex-alpha\",\"session_id\":\"sess-1\",\"host\":\"https://fort.example.test\",\"container_host\":\"http://fort.internal:8088\",\"access_token_path\":\"" + strings.ReplaceAll(paths.AccessTokenHostPath, "\\", "\\\\") + "\",\"refresh_token_path\":\"" + strings.ReplaceAll(paths.RefreshTokenHostPath, "\\", "\\\\") + "\",\"access_expires_at\":\"1970-01-01T00:00:00Z\",\"refresh_expires_at\":\"2030-01-01T00:00:00Z\",\"updated_at\":\"2030-01-01T00:00:00Z\"}'\n  exit 0\nfi\nif [ \"$1\" = \"fort\" ] && [ \"$2\" = \"session-state\" ] && [ \"$3\" = \"classify\" ]; then\n  printf '%s\\n' '{\"Revoked\":{\"snapshot\":{\"profile_id\":\"alpha\"},\"reason\":\"RefreshUnauthorized\"}}'\n  exit 0\nfi\nexit 1\n"
	if err := os.WriteFile(scriptPath, []byte(script), 0o700); err != nil {
		t.Fatalf("write script: %v", err)
	}
	t.Setenv(siRustCLIBinEnv, scriptPath)
	...
	if _, err := ensureUsableCodexProfileFortSession(context.Background(), profile, paths); err == nil {
		t.Fatalf("expected revoked rust classification to stop reuse")
	}
	if refreshCalls != 0 {
