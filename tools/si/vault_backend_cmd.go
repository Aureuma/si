...
func cmdVaultBackendStatus(args []string) {
	...
	report := map[string]any{
		...
		"helia_backup_enabled": resolution.Mode == vaultSyncBackendHelia || resolution.Mode == vaultSyncBackendDual,
		"helia_backup_strict":  resolution.Mode == vaultSyncBackendHelia,
		"sun_backup_enabled":   resolution.Mode == vaultSyncBackendHelia || resolution.Mode == vaultSyncBackendDual,
		"sun_backup_strict":    resolution.Mode == vaultSyncBackendHelia,
	}
	if *jsonOut {
		printJSON(report)
		return
	...
	fmt.Printf("%s %s\n", styleHeading("sun_backup_enabled:"), boolString(resolution.Mode == vaultSyncBackendHelia || resolution.Mode == vaultSyncBackendDual))
	fmt.Printf("%s %s\n", styleHeading("sun_backup_strict:"), boolString(resolution.Mode == vaultSyncBackendHelia))
}

func cmdVaultBackendUse(args []string) {
	settings := loadSettingsOrDefault()
	fs := flag.NewFlagSet("vault backend use", flag.ExitOnError)
	modeFlag := fs.String("mode", "", "vault sync backend mode: git, sun, helia, or dual")
	if err := fs.Parse(args); err != nil {
		fatal(err)
	}
	if fs.NArg() > 0 {
		printUsage("usage: si vault backend use --mode <git|sun|helia|dual>")
		return
	}
