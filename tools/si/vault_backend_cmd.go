...
func cmdVaultBackendStatus(args []string) {
	...
	fmt.Printf("%s %s\n", styleHeading("helia_backup_enabled:"), boolString(resolution.Mode == vaultSyncBackendHelia || resolution.Mode == vaultSyncBackendDual))
	fmt.Printf("%s %s\n", styleHeading("helia_backup_strict:"), boolString(resolution.Mode == vaultSyncBackendHelia))
}

func cmdVaultBackendUse(args []string) {
	settings := loadSettingsOrDefault()
	fs := flag.NewFlagSet("vault backend use", flag.ExitOnError)
	modeFlag := fs.String("mode", "", "vault sync backend mode: git, helia, or dual")
	if err := fs.Parse(args); err != nil {
		fatal(err)
	}
	if fs.NArg() > 0 {
		printUsage("usage: si vault backend use --mode <git|helia|dual>")
		return
	}
